...
public class SystemIdentification {
	//������ʶ��
	public static ArrayList<Point> pointSystem(GridData feature){
		ArrayList<Point> outputData=new ArrayList<Point>();
		
		for(int j=0;j<feature.gridInfo.nlat;j++)   //γ��ѭ��
		{
			for(int i=0;i<feature.gridInfo.nlon;i++)   //����ѭ��
			{
				if(Math.abs( feature.dat[i][j]-0.0f)<1e-4)          //��ȡ������
				{
					float lonNeed=feature.gridInfo.startlon+feature.gridInfo.dlon*i;   //�����㾭��
					float latNeed=feature.gridInfo.startlat+feature.gridInfo.dlat*j;      //������γ��
					outputData.add(new Point(lonNeed,latNeed));                           //����������
				}
			}
		}	
		return outputData;        //��������
	...
	//��ȡ��ѹ��������������ѹ������0.0��ʾ�����ಿ����1.0��ʾ
	public static GridData GenHighCenter(GridData heightData)
	{	
	    GridData inputData=heightData.copy();    //�����߶ȳ�
		inputData.smooth(20);                             //�߶ȳ�ƽ������
		GridData outputData=inputData.copy();    //�����
		outputData.setValue(1.0f);                        //����ΪĬ��ֵ
		float fMinScale=2.5f;                               //Ĭ����С�����߶�
		int   iMinSacle=(int)(fMinScale/inputData.gridInfo.dlon);
		
		for(int j=iMinSacle;j<inputData.gridInfo.nlat-iMinSacle;j++)   //γ��ѭ��
		{
			for(int i=iMinSacle;i<inputData.gridInfo.nlon-iMinSacle;i++)   //����ѭ��
			{
				int flagArea=0;                                     //��Χ��Ƿ���	
				int flagExtrem=0;                                  //��ֵ��Ƿ���
				int ii,jj=0;				
			    jj=j-iMinSacle;			
				for(ii=i-iMinSacle;ii<=i+iMinSacle;ii++)          
				{
					if(inputData.dat[ii][jj]>=inputData.dat[i][j])   //����е���ڸ����ĵ�
					{
						flagArea=flagArea+1;
					}						
				}
				ii=i+iMinSacle;
				for(jj=j-iMinSacle;jj<=j+iMinSacle;jj++)          
				{
					if(inputData.dat[ii][jj]>=inputData.dat[i][j])   //����е���ڸ����ĵ�
					{
						flagArea=flagArea+1;
				...
				for(jj=j-1;jj<=j+1;jj++)
				{
				    for(ii=i-1;ii<=i+1;ii++)          
				    {
				    	if(!(ii==i&&jj==j))                                         //�������
				    	{
					        if(inputData.dat[ii][jj]>=inputData.dat[i][j])   //����е���ڸ����ĵ�
					        {
						        flagExtrem=flagExtrem+1;
					        }
				    	}
				    }
				}
				if(flagArea==0&&flagExtrem==0)                      //��������������ĵ��Ǹ�ѹ���ĵ�
				{
					outputData.dat[i][j]=0.0f;          
				}								
			}
		}
		return outputData;
	...
	//��ȡ��ѹ��������������ѹ������0.0��ʾ�����ಿ����1.0��ʾ
	public static GridData GenLowCenter(GridData heightData)
	{	
	    GridData inputData=heightData.copy();    //�����߶ȳ�
		inputData.smooth(20);                             //�߶ȳ�ƽ������
		GridData outputData=inputData.copy();    //�����
	...
	public static Point[] GetHighLowStrenth(Point[] centerPoint, GridData height )
	{
		Point[] output=new Point[centerPoint.length];            //�����
		
		for(int n=0;n<centerPoint.length;n++)
		{
			int jy=(int)((centerPoint[n].ptLat-height.gridInfo.startlat)/(height.gridInfo.dlat));
			int ix=(int)((centerPoint[n].ptLon-height.gridInfo.startlon)/(height.gridInfo.dlon));
			
			float fValLatMean=0.0f;
			
			for(int i=0;i<height.gridInfo.nlon;i++)
			{
				fValLatMean=fValLatMean+height.dat[i][jy];
			}
			fValLatMean=fValLatMean/height.gridInfo.nlon;
			
		    output[n]=new Point(centerPoint[n].ptLon,centerPoint[n].ptLat,centerPoint[n].ptVal-fValLatMean);
		}		
		return output;		
	}
	
	//获取线状系统
		public static ArrayList<Line> lineSystem(float scale,GridData feature,GridData marker0,VectorData tranWind){
			...
			for(int i=0;i<1;i++){
				...
				GridData adveOfAdve= VectorMathod.getAdvection(tranDirection, adve); //计算平流的平流，在槽线位置平流的平流为正，脊线位置平流的平流为负
				GridData minalAofA=adveOfAdve.mutiply(-1).sign01();                    //为提取脊线部分，保留平流的平流为负的区域
				minalAofA.writeToFile("G:/data/systemIdentify/maOfa.txt");
				GridData marker=marker1.mutiply(minalAofA);					 //生成marker场
				ArrayList<Line> cutedTrough=LineDealing.cutLines(origainalTrough, marker);  //裁剪获得脊线
				LineDealing.writeToFile("G:/data/systemIdentify/cuttroughs.txt", cutedTrough);
				trough = getLongLine(scale,cutedTrough);
				LineDealing.smoothLines(trough, 100);
				LineDealing.writeToFile("G:/data/systemIdentify/troughs.txt", trough);
				tranDirection = getDirectionFromLine(trough,marker1);
				tranDirection.writeToFile("G:/data/systemIdentify/trainWind1.txt");
				System.out.println();
			}
			
			trough = getLongLine(scale,trough);
			return trough;
		...
		static ArrayList<Line> getLongLine(float scale, ArrayList<Line> lines) {
			...
			for(Line line: lines ){
				...
				if(lineLenght*111.0f>scale) longLines.add(line);
			}
			return longLines;
		}
