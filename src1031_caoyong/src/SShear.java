...
public class SShear{
	...
	public static WeatherSystems getShear(VectorData wind, int level, float scale) {
		...
		for(int k=0;k<2;k++){
			directionShear = getMeanDirectionShear(tranDirection,wind);
			marker = directionShear.add(-0.5f).sign01();
			directionShear.writeToFile("G:/data/systemIdentify/directionShear.txt");
			shearLine = SystemIdentification.HighValueAreaRidge(tranDirection, directionShear,level);
			shearLine = LineDealing.cutLines(shearLine, marker);
			LineDealing.smoothLines(shearLine, 10);
			shearLine = SystemIdentification.getLongLine(scale,shearLine);
			tranDirection = SystemIdentification.getDirectionFromLine(shearLine, directionShear.sign01());
		}
		...
		GridData windShear = getMeanWindShear(tranDirection,wind);
		...
		GridData shearFeature = windShear.add(lineDiv).mutiply(marker);
		shearFeature.smooth(3);
		
		marker = VV.add(-0.3f).sign01().mutiply(minShear.add(0.3f).sign01()).mutiply(shearFeature.sign01());
		...
		WeatherSystems shear = new WeatherSystems("shear",level);
	...
	private static GridData getLineVV(VectorData ptranWind, VectorData wind) {
		...
		//GridData VV = upperV.mutiply(downV);
	...
	public static void getShearStrenght(ArrayList<Line> line, GridData grid){
		...
		for(int i = 0; i < line.size(); i++){
			...
			for(int j = 0; j < line.get(i).point.size()-1; j++){			
				...
				value = VectorMathod.getValue(grid, point);
		//		line.get(i).strenght += value;
		//		line.get(i).area +=0 ;
		//		if(value>line.get(i).maxFeature)line.get(i).maxFeature = maxStrenght;
			}
		}
