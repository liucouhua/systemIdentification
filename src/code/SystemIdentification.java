...
import code.GridData;
import code.GridInfo;
import code.MyMath;
...
/*
		...
		*/
		...
		public static ArrayList<GridData> getCuttedRegion_3d(ArrayList<GridData> oriData){
			// 根据格点场分布，将大于0的区域按梯度上升法切割成各个极值点的覆盖分区
			if(oriData.size()<=0)return null;
			ArrayList<GridData> crId = new ArrayList<GridData>();
			for(int k=0;k<oriData.size();k++){
				crId.add(new GridData(oriData.get(0).gridInfo));
			}
			int nobj=0;
			ArrayList<ArrayList<GridData>> grad = getGradsDirection(oriData);
			int nlon=oriData.get(0).gridInfo.nlon;
			int nlat=oriData.get(0).gridInfo.nlat;
			int ntime = oriData.size();
			
			class GrowPoint{
				int i,j,t;
				public GrowPoint(int i0,int j0,int t0){
					i=i0;j=j0;t=t0;
				}
			}
			Queue<GrowPoint> queue=new LinkedList<GrowPoint>(); 
			...
			float u,v,w;
			int i1,j1,t1,i2,j2,t2;
			//先对所有极值点附近格点赋值
			for(int t=0;t<ntime;t++){
				for(int i=0;i<nlon;i++){
					for(int j=0;j<nlat;j++){
				        if(crId.get(t).dat[i][j]!=0||oriData.get(t).dat[i][j]<=0)continue;  
				        boolean isMax=true;
				        for(int r=-1;r<2;r++){
					        for(int p=-1;p<2;p++){
					        	for(int q=-1;q<2;q++){
					        		i1= MyMath.cycleIndex(false, nlon, i, p);
					        		j1= MyMath.cycleIndex(false, nlat, j, q);
					        		t1= MyMath.cycleIndex(false, ntime, t, r);
					        		if(oriData.get(t1).dat[i1][j1]>oriData.get(t1).dat[i][j]){
					        			isMax = false;
					        			break;
					        		}
					        	}
				        ...
				        for(int r=-1;r<2;r++){
					        for(int p=-1;p<2;p++){
					        	for(int q=-1;q<2;q++){
					        		...
					        		j1= MyMath.cycleIndex(false, nlat, j, q);
					        		t1= MyMath.cycleIndex(false, ntime, t, r);
					        		if(oriData.get(t1).dat[i1][j1]>0&&crId.get(t1).dat[i1][j1]==0){
					        			crId.get(t1).dat[i1][j1]=nobj;   //将极值中心点附近27个点都赋为同一编号
					     		        queue.offer(new GrowPoint(i1,j1,t1));
					        		}
					        	}
					        }
				        }
		    ...
		    while((gp=queue.poll())!=null){
		    	...
		    	for(int r=-1;r<2;r++){
			        for(int p=-1;p<2;p++){
			        	for(int q=-1;q<2;q++){
			        		...
			        		i2= MyMath.cycleIndex(false, nlon, i1, p);
			        		j2= MyMath.cycleIndex(false, nlat, j1, q);     
			        		t2= MyMath.cycleIndex(false, nlat, t1, q); 
			        		if(oriData.get(t2).dat[i2][j2]>0&&crId.get(t2).dat[i2][j2]==0){
			        			crId.get(t2).dat[i2][j2] =-1;
			     		        queue.offer(new GrowPoint(i2,j2,t2));
			        		}
			        	}
			        }
			    }
		        ...
		        point[2] = t1;
		        int step =0;
		        while(crId.get(t1).dat[i1][j1]<=0){
		        	
		        	u= VectorMathod.getValue(grad.get(0),point);
					v= VectorMathod.getValue(grad.get(1),point);
					w= VectorMathod.getValue(grad.get(2),point);
					float random = (float) Math.random()*0.2f;
					if(u<0 && point[0]<1)u=-Math.abs(random*v);
					if(u>0 && point[0]>nlon-2)u=Math.abs(random*v);
					if(v<0 && point[1]<1)v=-Math.abs(random*u);
					if(v>0 && point[1]>nlat-2)v = Math.abs(random*u);
					if(w<0 && point[2]<1)w=-Math.abs(random*u);
					if(w>0 && point[1]>ntime-2)w = Math.abs(random*u);
					if(u==0 && v == 0 && w==0) u=1;
					speed = (float) Math.sqrt(u * u + v * v + w*w)+0.000001f;
					point[0] += u / speed;
					point[1] += v / speed;
					point[2] += w / speed;
					if(point[0]<=0)point[0]=0.001f;
					...
					if(point[1]<=0)point[1]=0.001f;
					if(point[1]>=nlat-1) point[1] = nlat-1.001f;
					if(point[2]<=0)point[2]=0.001f;
					if(point[2]>=ntime-1) point[2] = ntime-1.001f;
					ig = (int)point[0];
					jg = (int)point[1];
					tg = (int)point[2];
					
					//将矩形框8个顶点中所有非0id找出来，如果他们相同则表示到达了分区范围
					float sameId=0;
					boolean isSame =true;
			    	for(int r=0;r<2;r++){
				        for(int p=0;p<2;p++){
				        	for(int q=0;q<2;q++){
				        		if(crId.get(tg+r).dat[ig+p][jg+q]!=sameId){
				        			if(sameId ==0) {
				        				sameId = crId.get(tg+r).dat[ig+p][jg+q];
				        			}
				        			else if(crId.get(tg+r).dat[ig+p][jg+q]>0){
				        				isSame = false;
				        				break;
				        			}
				        		}
					...
					step++;
					if(step>nlon+nlat+ntime){
						crId.get(t1).dat[i1][j1] = sameId;
						break;
					}
					if(!isSame) sameId =0;
					crId.get(t1).dat[i1][j1] = sameId;
			    ...
			    for(int i=0;i<nlon;i++){
					for(int j=0;j<nlat;j++){
						Map<Integer,Integer> idNum=new HashMap<Integer,Integer>();
						 for(int p=-4;p<5;p++){
					        	for(int q=-4;q<5;q++){
					        		...
					        		if(idNum.containsKey(key)&&key!=0){
						...
						for (Map.Entry<Integer,Integer> entry : idNum.entrySet()){
							if(entry.getValue()>maxNum&&crId.dat[i][j]!=0){
		...
		public static void smoothIds_3d(ArrayList<GridData> crId){
			int nlon=crId.get(0).gridInfo.nlon;
			int nlat=crId.get(0).gridInfo.nlat;
			int ntime = crId.size();
			int i1,j1,t1;
			ArrayList<GridData> smId = new ArrayList<GridData>();
			for(int t=0;t<ntime;t++){
				smId.add(new GridData(crId.get(0).gridInfo));
			}
		    ...
		    for (int k=0;k<2;k++){
						...
						for(int j=0;j<nlat;j++){
							 ...
							 for(int r=-4;r<5;r++){
								 for(int p=-4;p<5;p++){
						        	for(int q=-4;q<5;q++){
						        		...
						        		j1= MyMath.cycleIndex(false, nlat, j, q);
						        		t1= MyMath.cycleIndex(false, ntime, t, r);
						        		int key =(int)crId.get(t1).dat[i1][j1];
						        		if(idNum.containsKey(key)&&key!=0){
						        			idNum.put(key, idNum.get(key)+1);
						        		}
						        		else{
							...
							for (Map.Entry<Integer,Integer> entry : idNum.entrySet()){
								if(entry.getValue()>maxNum){
									maxNum = entry.getValue();
									smId.get(t).dat[i][j] =entry.getKey();
								}
							}	 
						}
				    }
		    	...
		    	for(int t=0;t<ntime;t++){
				    for(int i=0;i<nlon;i++){
						for(int j=0;j<nlat;j++){
							crId.get(t).dat[i][j] = smId.get(t).dat[i][j];
						}
				    }
				}
		    }
		...
		private static ArrayList<ArrayList<GridData>> getGradsDirection(ArrayList<GridData> gridList) {
			// TODO Auto-generated method stub
			ArrayList<ArrayList<GridData>> xytList =  new ArrayList<ArrayList<GridData>>();
			GridInfo gridInfo = gridList.get(0).gridInfo;
			int nlat=gridList.get(0).gridInfo.nlat;
			int nlon=gridList.get(0).gridInfo.nlon;
			int ntime= gridList.size();
			int t_1,t1,dt,j_1,j1,i_1,i1,dj,di;
			ArrayList<GridData> xList =new ArrayList<GridData>();
			ArrayList<GridData> yList =new ArrayList<GridData>();
			ArrayList<GridData> tList =new ArrayList<GridData>();
			
			for(int t=0; t < ntime; t++){
				t_1=MyMath.cycleIndex(false, ntime, t, -1);
				t1=MyMath.cycleIndex(false, ntime, t, 1);
				dt=t1-t_1;
				GridData u = new GridData(gridInfo);
				GridData v = new GridData(gridInfo);
				GridData w = new GridData(gridInfo);
				for(int j=0;j<nlat;j++){
					j_1=MyMath.cycleIndex(false, nlat, j, -1);
					j1=MyMath.cycleIndex(false, nlat, j, 1);
					dj=j1-j_1;
					for(int i=0;i<nlon;i++){
						...
						i1=MyMath.cycleIndex(false, nlon, i, 1);
						di=i1-i_1;
						u.dat[i][j]=(gridList.get(t).dat[i1][j]-gridList.get(t).dat[i_1][j])/di;
						v.dat[i][j]=(gridList.get(t).dat[i][j1]-gridList.get(t).dat[i][j_1])/dj;
						w.dat[i][j]=(gridList.get(t1).dat[i][j]-gridList.get(t_1).dat[i][j])/dt;
						float mod = (float) Math.pow(u.dat[i][j]*u.dat[i][j]+v.dat[i][j]*v.dat[i][j]+w.dat[i][j]*w.dat[i][j],0.5f);
						u.dat[i][j]/= mod;
						v.dat[i][j]/= mod;
						w.dat[i][j]/= mod;
					}
				...
				tList.add(w);xList.add(u); yList.add(v); 
			}
