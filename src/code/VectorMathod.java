...
public class VectorMathod {
	...
	static float getValue(ArrayList<GridData> grid, float[] point) {
		// TODO Auto-generated method stub
		int ig=(int) ((point[0]-grid.get(0).gridInfo.startlon)/grid.get(0).gridInfo.dlon);
		int jg=(int) ((point[1]-grid.get(0).gridInfo.startlat)/grid.get(0).gridInfo.dlat);
		int tg=(int) (point[2]);
		float dx=((point[0]-grid.get(0).gridInfo.startlon)/grid.get(0).gridInfo.dlon)-ig;
		float dy=((point[1]-grid.get(0).gridInfo.startlat)/grid.get(0).gridInfo.dlat)-jg;
		float dt = point[2]-tg;
		float c000=(1-dx)*(1-dy)*(1-dt);
		float c100=dx*(1-dy)*(1-dt);
		float c010=(1-dx)*dy*(1-dt);
		float c110=dx*dy*(1-dt);
		float c001=(1-dx)*(1-dy)*dt;
		float c101=dx*(1-dy)*dt;
		float c011=(1-dx)*dy*dt;
		float c111=dx*dy*dt;
		
		
		float value;
		if(ig>=0&&ig<grid.get(0).gridInfo.nlon-1
				&&jg>=0&&jg<grid.get(0).gridInfo.nlat-1
				&&tg>=0&&tg<grid.size()-1)
		{
				value=c000*grid.get(tg).dat[ig][jg]+c100*grid.get(tg).dat[ig+1][jg]+
						c010*grid.get(tg).dat[ig][jg+1]+c110*grid.get(tg).dat[ig+1][jg+1]+
						c001*grid.get(tg+1).dat[ig][jg]+c101*grid.get(tg+1).dat[ig+1][jg]+
						c011*grid.get(tg+1).dat[ig][jg+1]+c111*grid.get(tg+1).dat[ig+1][jg+1];	
		}
		else{
			value=9999;
		}
