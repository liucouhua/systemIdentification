...
public class GridInfo implements Serializable{
	...
	public GridInfo(GridDataModel model){
		if(model.getLatstep()<0){
			this.startlat=(float) model.getEndLat();
			this.endlat=(float) model.getStartLat();
		}else{
			this.startlat=(float) model.getStartLat();
			this.endlat=(float) model.getEndLat();
		}
		if(model.getLonstep()<0){
			this.startlon=(float) model.getEndLon();
			this.endlon=(float) model.getStartLon();
		}else{
			this.startlon=(float) model.getStartLon();
			this.endlon=(float) model.getEndLon();
		}
		this.dlon=(float) Math.abs(model.getLonstep());
		this.dlat=(float) Math.abs(model.getLatstep());
		this.nlon=model.getLonCount();
		this.nlat=model.getLatCount();
	}
